...
export default function CardManagementScreen({
  ...
  useEffect(() => {
    ...
    const storedRounds = JSON.parse(
      localStorage.getItem("dailyRounds") || "{}"
    );
  ...
  const handleRefresh = async () => {
    try {
      ...
      const res = await fetch(`https://gojoapi.onrender.com/round/${shopId}`);
  ...
  const calculatePrize = () => {
    ...
    const rate = commission_rate != null ? parseFloat(commission_rate) : 0.2; // default 10% if undefined
      ...
      <main className="flex-1 flex flex-col items-center mt-6">
        ...
        <div className="flex items-start gap-10">
          ...
          <div className="flex flex-col items-start text-blue-400 space-y-4">
            <p>
              award:{" "}
              <span className="text-white font-bold">{calculatePrize()}</span>
            </p>
